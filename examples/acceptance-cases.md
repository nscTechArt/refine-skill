...
This creates six fresh repositories outside this checkout and prints their root and requests. Each has a local fixture commit, staged changes where appropriate, and `python -B checks.py` as its behavior check. It does not stage or commit this plugin's files.

Give an independent evaluator only the installed skill path, one fixture directory, and its printed request. Keep the harness and the parent `manifest.json` out of the evaluator's context. Permit mutations only in that fixture. Use normal three-reviewer delegation if capacity allows; otherwise require the skill's disclosed sequential fallback. Collect its final response and tool trace.
